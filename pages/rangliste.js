...
export default function RanglistePage({ board, currentUserId }) {
  return (
    <>
        ...
        <div className={s.wrap}>
          ...
          <div className={s.tableWrap}>
            <table className={s.table}>
              ...
              <tbody>
                {board.map((p, i) => (
                  <tr key={p.id} className={`${s.tr}${p.id === currentUserId ? " " + s.trMe : ""}`}>
...
export async function getServerSideProps(context) {
  ...
  return { props: { board, currentUserId: session.user.id } };
}
